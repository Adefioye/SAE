...
def test_report_from_synthetic_cached_artifacts(tmp_path: Path) -> None:
    ...
    assert (store.root / "plots" / "canonical_correlation_spectra.png").exists()
    assert (store.root / "plots" / "pca_explained_variance_curves.png").exists()
    assert (
        store.root / "plots" / "paper_figure_1_encoder_decoder_alignment.png"
    ).exists()
    assert (store.root / "plots" / "paper_figure_a1_threshold_sweep.png").exists()
    assert (
        store.root / "plots" / "paper_figure_a2_decoder_hungarian_vs_max_cosine.png"
    ).exists()
    assert (store.root / "plots" / "paper_shared_orphan_fractions.png").exists()
    assert not list((store.root / "plots").glob("*.svg"))
